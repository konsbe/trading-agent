import { DEFAULT_MOMENTUM_API_URL, getMomentumApiBaseUrl, SCANNER_ENDPOINTS } from './api.config';
...
describe('api.config', () => {
    ...
    it('prefers the hosted shell config over the build-time default', () => {
        process.env.MOMENTUM_API_URL = 'http://build-time:1234';
        window.__APP_CONFIG__ = { shell_spog: { config: { momentumApiUrl: 'http://hosted:8090' } } };
        expect(getMomentumApiBaseUrl()).toBe('http://hosted:8090');
    });
    ...
    it('builds endpoint paths', () => {
        expect(SCANNER_ENDPOINTS.today).toBe('/api/v1/scanner/today');
        expect(SCANNER_ENDPOINTS.symbol('NEXR')).toBe('/api/v1/scanner/today/NEXR');
    });
});
