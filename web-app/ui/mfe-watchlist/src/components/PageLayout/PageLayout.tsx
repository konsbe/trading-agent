import { DisclaimerPill } from '@trading-agent/shared-components';
import { useIsHosted } from '@/providers/HostModeContext';
import { PageLayoutProps } from './types';
import './PageLayout-styles.css';

/**
 * Page frame for every watchlist screen. The disclaimer pill is rendered here
 * only when standalone; hosted, spog's header carries it on every screen.
 */
const PageLayout = ({ title, subtitle, actions, backLink, children }: PageLayoutProps) => {
    const isHosted = useIsHosted();
    const showPill = !isHosted;
    const hasHeader = Boolean(title || subtitle || actions || showPill);

    return (
        <div className={`watchlist-page${isHosted ? ' watchlist-page--hosted' : ''}`} data-testid="watchlist-page">
            {backLink && <nav className="watchlist-page__back" aria-label="Breadcrumb">{backLink}</nav>}
            {hasHeader && (
                <header className="watchlist-page__header">
                    <div className="watchlist-page__heading">
                        {title && <h1 className="watchlist-page__title">{title}</h1>}
                        {subtitle && <div className="watchlist-page__subtitle">{subtitle}</div>}
                    </div>
                    <div className="watchlist-page__actions">
                        {actions}
                        {showPill && <DisclaimerPill />}
                    </div>
                </header>
            )}
            <div className="watchlist-page__body">{children}</div>
        </div>
    );
};

export default PageLayout;
