...
describe('ApiErrorState', () => {
    it('shows the mapped message, the code and HTTP status, and retries', async () => {
        ...
        render(<ApiErrorState error={{ status: 503, code: 'database_unavailable' }} onRetry={onRetry} />);

        const alert = screen.getByRole('alert');
        expect(alert).toHaveTextContent('The watchlist database is unavailable right now.');
        expect(alert).toHaveTextContent('database_unavailable · HTTP 503');
