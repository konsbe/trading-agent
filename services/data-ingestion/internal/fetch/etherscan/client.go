...
func (c *Client) EthSupply(ctx context.Context) (float64, error) {
	...
	u := "https://api.etherscan.io/v2/api?" + q.Encode()
	...
	if body.Status != "1" {
		return 0, fmt.Errorf("etherscan: %s (%s)", body.Message, body.Result)
	}
	i := new(big.Int)
	if _, ok := i.SetString(body.Result, 10); !ok {
		return 0, fmt.Errorf("etherscan: unexpected result format: %q", body.Result)
	}
