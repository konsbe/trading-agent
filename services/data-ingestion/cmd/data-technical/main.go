...
type worker struct {
	...
	pool  *pgxpool.Pool
	yahoo *yahoo.Client      // primary source for equity daily/weekly bars (free, no key)
	alp   *alpacadata.Client // retained for hourly bars and as fallback
	bin   *binance.REST
	log   *slog.Logger
}

// backfill ensures each symbol × interval combination has at least BackfillBars
// rows in the database. Existing rows are preserved via ON CONFLICT DO UPDATE.
//
// Equity bars: Yahoo Finance is tried first (free, no key, 2-year history).
// If Yahoo returns no bars, Alpaca is used as a fallback (requires credentials).
func (w *worker) backfill(ctx context.Context) {
	for _, sym := range w.cfg.EquitySymbols {
		for _, iv := range w.cfg.EquityIntervals {
			...
			// Fall back to Alpaca if Yahoo returned nothing.
			...
			if err := store.UpsertEquityOHLCV(ctx, w.pool, bars); err != nil {
				...
				w.log.Info("equity backfill done", "symbol", sym, "interval", iv, "bars", len(bars), "source", bars[0].Source)
			}
		}
	}
...
// fetchLatest pulls the most recent bars so the DB stays current between polls.
// Yahoo Finance is used for equity bars; Alpaca is the fallback.
...
// ts is set to the timestamp of the last bar — indicators are anchored to
// bar-close time so re-running the same data is idempotent.
func (w *worker) computeAndStore(ctx context.Context, bars []compute.Bar, symbol, exchange, interval string) {
	...
	// ── MACD (current + prior bar for crosses; no extra DB read) ─────────────
	if w.cfg.EnableMACD {
	...
	if w.cfg.EnableOBV {
		if total, dlt, ok := compute.OBVLast(bars); ok {
			upsert("obv", ptr(total), map[string]any{
				"obv":               total,
				"last_bar_delta":    dlt,
				"cumulative_series": "full_series_not_stored",
			})
		}
	}
	...
	if w.cfg.EnableFib {
		...
		if fib, ok := compute.FibRetracementFromSwings(highs, lows, closes, fs, w.cfg.FibExtensions); ok {
			...
			upsert(fname, ptr(fib.DistPctToNear), map[string]any{
				"rule":              "last_swing_high_vs_last_swing_low; impulse direction by which pivot is more recent",
	...
	// ── RSI divergence (swing pivots vs RSI Wilder series) ───────────────────
	if w.cfg.EnableRSIDivergence {
		...
		upsert(dname, ptr(compute.RSIDivergenceScore(div)), map[string]any{
			...
			"bearish_regular": map[string]any{
				...
				"idx_hi_1":   div.HighIdx1,
				"idx_hi_2":   div.HighIdx2,
			},
			"bullish_regular": map[string]any{
				...
				"idx_lo_1":   div.LowIdx1,
				"idx_lo_2":   div.LowIdx2,
			},
		})
	}

	// ── RSI hidden divergence (continuation; stricter pivots) ───────────────
	if w.cfg.EnableRSIHidden {
		...
		upsert(hname, ptr(compute.RSIHiddenScore(hid)), map[string]any{
			...
			"bearish_hidden": map[string]any{
				...
				"idx_hi_1":   hid.HighIdx1,
				"idx_hi_2":   hid.HighIdx2,
			},
			"bullish_hidden": map[string]any{
				...
				"idx_lo_1":   hid.LowIdx1,
				"idx_lo_2":   hid.LowIdx2,
			},
	...
	// ── Volume profile proxy (bar-only; not tape/L2) ─────────────────────────
	if w.cfg.EnableVolProfileProxy && w.cfg.VolProfileBins >= 2 {
		if bins, poc, pocIdx, ok := compute.VolumeProfileProxy(bars, w.cfg.VolProfileBins, w.cfg.VolProfileTypical); ok {
			...
			rows := make([]map[string]float64, len(bins))
			for i, b := range bins {
	...
	// ── Tier 3: Accumulation / Distribution line ────────────────────────────
	...
	// ── Tier 3: Pivot pack (prior bar = reference session) ───────────────────
		...
		case "session":
			if v, day, ok := compute.VWAPSessionLastDay(bars, useTyp); ok {
				upsert("vwap_session_last_day", ptr(v), map[string]any{
					...
					"note": "Uses all bars in window sharing the last bar's UTC date; for daily bars equals that bar only.",
				})
			}
		default:
	...
	// ── Tier 3: MA ribbon + golden/death cross ──────────────────────────────
	...
	// ── Tier 3: Chart-pattern hints (heuristic) ───────────────────────────────
	...
	// ── Trendline break primitives (OLS on swing pivots) ───────────────────────
	...
	// ── Market structure (BOS / swing-break hints) ────────────────────────────
	...
	// ── Gann (regression slope proxy; not full square-of-nine) ────────────────
	...
	// ── Open interest: document data gap (OHLCV has no OI) ────────────────────
	if w.cfg.EnableOpenInterestInfo {
		upsert("open_interest", nil, map[string]any{
			"available": false,
			"reason":    "Open interest is not part of OHLCV; add a futures/options OI feed and ingestor to populate this.",
		})
	}
...
// formatFloatKey turns 2.0 into "2", 2.5 into "2.5" for stable indicator names.
