...
// they lived in equity_ohlcv and universe_symbols, then the eligibility
...
func TestReferenceSeries_AreNotInEquityOHLCV(t *testing.T) {
	ctx := context.Background()
	pool := testPool(t)
	...
	for _, id := range refSeriesIDs() {
		var n int
		if err := pool.QueryRow(ctx,
			`SELECT count(*) FROM equity_ohlcv WHERE symbol = $1`, id).Scan(&n); err != nil {
			t.Fatalf("query: %v", err)
		}
		if n != 0 {
			t.Errorf("%s has %d row(s) in equity_ohlcv; reference series belong in reference_series, "+
				"where nothing joins them to the universe", id, n)
		}
	}
}
